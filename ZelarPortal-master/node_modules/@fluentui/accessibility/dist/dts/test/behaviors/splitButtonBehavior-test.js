...
describe('SplitButtonBehavior.ts', function () {
    ...
    test('close menu with different keys', function () {
        ...
        var supportedKeys = [
            ...
            { altKey: true, keyCode: accessibility_1.keyboardKey.ArrowUp },
            { keyCode: accessibility_1.keyboardKey.Tab, shiftKey: false },
            { keyCode: accessibility_1.keyboardKey.Tab, shiftKey: true },
        ];
        var keysFromBehavior = accessibility_1.splitButtonBehavior(property)['childBehaviors']['menuButton'](property).keyActions.popup.closeAndFocusTrigger
            .keyCombinations;
        verifyKeys(supportedKeys, keysFromBehavior);
    });
    test('open menu with alt + arrow down ', function () {
        var property = {};
        var supportedKeys = [{ altKey: true, keyCode: accessibility_1.keyboardKey.ArrowDown }];
        var keysFromBehavior = accessibility_1.splitButtonBehavior(property)['childBehaviors']['menuButton'](property).keyActions.root.openAndFocusFirst
            .keyCombinations;
        verifyKeys(supportedKeys, keysFromBehavior);
    });
});
